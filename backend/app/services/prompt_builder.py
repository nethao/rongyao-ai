...
7. **排版要求（WordPress 适配）**
   - 输出内容将用于在 WordPress 中发布，需适配 WordPress 的排版风格
   - 若原文来自公众号、美篇等富排版来源：必须抛弃堆叠的样式、背景图、装饰性背景、复杂嵌套版式等，仅保留正文信息与图片
   - **保留标题层级**：原文中的一级标题、二级标题、三级标题等必须保留，使用 HTML 标签（<h2> <h3> <h4>）
   - **段落格式**：
     * 正文段落使用 <p> 标签包裹
     * 每个正文段落添加首行缩进样式：<p style="text-indent: 2em;">段落内容</p>
     * 标题段落不需要首行缩进
     * **图片占位符不要包裹任何标签**：[IMAGE_PLACEHOLDER_N] 单独成行，前后不要添加 <p> 或其他标签
   - 使用简洁、扁平的结构：段落、标题、列表、图片，不要保留或模仿原文中的装饰性排版描述
...
- 不要使用"转换后："、"修改为："等前缀
- 不要添加"以上是转换结果"等后缀
- 只返回纯净的、适合 WordPress 发布的转换后文本内容
- **关键**：[IMAGE_PLACEHOLDER_N] 占位符必须保持在原文中的相对位置，不要移动到文末"""
