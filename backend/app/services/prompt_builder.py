...
- 规范化时间表述
- 人称替换逻辑（我局/我司 -> 来源）
- 排版:WordPress 适配，公众号/美篇来源需抛弃堆叠样式、背景图等
"""
import logging
...
3. **图片占位符处理（极其重要）**
   - 文本中包含 [[IMG_N]] 格式的占位符，这些是图片位置标记
   - **必须保留所有占位符**:占位符代表图片位置，必须原样保留在转换后的文本中
   - **严格保持图片位置**:占位符在原文中出现在哪一行，转换后必须仍在对应位置，不得移动到文末或其他位置
   - **禁止移动占位符**:即使占位符在段落中间、文章开头、或看起来"不合适"的位置，也绝对不要移动
   - **特别注意**:如果原文第一行就是 [[IMG_1]]，转换后第一行也必须是 [[IMG_1]]，不要添加标题后再放图片
   - **示例**:
     * 原文: "[[IMG_1]]\n\n段落1\n\n段落2\n\n[[IMG_2]]\n\n段落3"
     * 正确: "[[IMG_1]]\n\n段落1\n\n段落2\n\n[[IMG_2]]\n\n段落3"
     * 错误: "段落1\n\n段落2\n\n段落3\n\n[[IMG_1]]\n\n[[IMG_2]]"
...
7. **输出格式要求（Markdown）**
   - 使用标准Markdown格式输出
   - **标题**:使用 ## 二级标题、### 三级标题、#### 四级标题
   - **段落**:段落之间用空行分隔，不需要特殊标记
   - **列表**:使用 - 或 1. 2. 3. 连续编号，不要跳号或插入空项
   - **图片占位符**:[[IMG_N]] 单独成行，前后用空行分隔
   - **强调**:使用 **粗体** 或 *斜体*
   - 不要使用HTML标签
   - 若原文来自公众号、美篇等富排版来源:必须抛弃堆叠的样式、背景图、装饰性背景、复杂嵌套版式等，仅保留正文信息与图片
   - 不输出与版式相关的说明（如"上方为头图""背景为红色"等），只输出可发布正文

**输出要求:**
- 直接返回转换后的Markdown文本，不要添加任何解释、说明或元信息
- 不要使用"转换后:"、"修改为:"等前缀
- 不要添加"以上是转换结果"等后缀
- 只返回纯净的Markdown格式内容
- **图片位置约束（必须遵守）**: 
  * 原文中 [[IMG_N]] 在哪个段落之间，转换后必须保持在相同段落之间
  * 禁止将所有图片集中到文末
  * 如果原文开头有图片，转换后开头也必须有图片
  * 如果原文中间有图片，转换后中间也必须有图片
- **列表格式**: 正确示例 "1. 项目A\n2. 项目B"，错误示例 "1. \n2. 项目A\n3. \n4. 项目B"

**特别提醒**: 图片占位符 [[IMG_N]] 的位置是图文排版的关键，移动图片位置会严重影响文章的可读性和美观度，请务必保持图片在原文中的相对位置。"""
